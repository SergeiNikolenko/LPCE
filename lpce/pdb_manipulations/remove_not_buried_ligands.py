...
def process_structures(output_dir, threshold=0.3, distance_cutoff=5.0, n_jobs=112):
    ...
    results = Parallel(n_jobs=n_jobs)(
        delayed(is_ligand_buried)(pdb_file, threshold, distance_cutoff) for pdb_file in tqdm(pdb_files)
    )
    ...
    logger.info(f"Deleted {len(removed_files)} files, which is {len(removed_files) / len(pdb_files) * 100:.2f}% of the total.")

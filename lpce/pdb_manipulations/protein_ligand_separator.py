...
def calculate_buried_fraction(structure, ligand_residues, pocket_info, threshold=0.5, radius=5.0):
    ligand_atoms = [atom for residue in ligand_residues for atom in residue.get_atoms()]
    protein_atoms = [
        atom for atom in structure.get_atoms()
        if atom.get_parent().get_parent().id in pocket_info['interacting_chains'] and atom.get_parent().id[0] == ' '
    ]
...
def calculate_aligned_rmsd(structure1_file, structure2_file, input_file_path, identity_threshold=0.9):
    ...
    aligned_seq1, aligned_seq2 = get_aligned_sequences(alignments[0], seq1_str, seq2_str)
...
def filter_conect_lines(conect_lines, saved_atoms):
    ...
    for line in conect_lines:
        if line.startswith("CONECT"):
            ...
            # Оставляем строку только если в ней остались валидные номера
            if valid_atom_numbers:
                filtered_conect_lines.append(f"CONECT{''.join(valid_atom_numbers)}\n")
    return filtered_conect_lines

def save_pocket_structure(structure, pocket_info, output_dir, saved_structures, original_lines, conect_lines, 
                          input_file_path, rmsd_threshold=2.0, identity_threshold=0.98):
    ...
    old_atom_serials = {atom.get_serial_number(): atom for atom in structure.get_atoms() if select.accept_atom(atom)}

    temp_structure_file = save_structure_to_tempfile(structure, select)

    similar_found = False
    for saved in saved_structures:
        rmsd_value = calculate_aligned_rmsd(saved['temp_file'], temp_structure_file, input_file_path, identity_threshold=identity_threshold)
    ...
    new_atom_serials = {atom.get_serial_number(): atom for atom in new_structure.get_atoms() if select.accept_atom(atom)}
    serial_map = {old_serial: new_serial for (old_serial, old_atom), (new_serial, new_atom) in zip(old_atom_serials.items(), new_atom_serials.items())}
...
def process_pdb_file(input_file_path, output_dir_path, interact_distance=4.5, ligand_ligand_distance=3.0, rmsd_threshold=2.0, identity_threshold=0.98):
...
def analyze_protein(input_file_path, output_dir_path='separated_complexes', interact_distance=4.5, ligand_ligand_distance=3.5, rmsd_threshold=2.0, identity_threshold=0.98):
    logger.debug(f"Starting analysis of protein: {input_file_path}")
    
    saved_structures = process_pdb_file(input_file_path, output_dir_path, interact_distance, ligand_ligand_distance, rmsd_threshold)
    
    saved_count = sum(1 for s in saved_structures if not s.get('skipped_similar', False))
    skipped_similar = sum(1 for s in saved_structures if s.get('skipped_similar', False))
...
def protein_ligand_separator(cfg):
    ...
    results = Parallel(n_jobs=-1)(
        delayed(analyze_protein)(pdb_file, output_dir, interact_distance, ligand_ligand_distance, rmsd_threshold, identity_threshold)
    ...
    total_structures_skipped_similar = sum(result['structures_skipped_similar'] for result in results)

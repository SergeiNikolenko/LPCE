...
def process_pdb_file(input_file, output_dir):
    """
    ...
    for ligand_id, lines in hetatm_lines.items():
        ...
        subprocess.run(['obabel', '-ipdb', str(ligand_pdb_file), '-osdf', '-O', str(ligand_sdf_file)],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        subprocess.run(['obabel', '-ipdb', str(ligand_pdb_file), '-omol2', '-O', str(ligand_mol2_file)],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def create_final_files(cfg):
    ...
    Parallel(n_jobs=cfg.n_jobs)(
        delayed(process_pdb_file)(pdb_file, output_path)
